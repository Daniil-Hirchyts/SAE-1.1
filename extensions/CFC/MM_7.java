...
public class MM_7 {
    ...
    public static char[] mancheOrdiCFC(int lgCode, int nbCouleurs, char[] tabCouleurs, int[] score) {
        char[] codeRef = mancheOrdi(lgCode, nbCouleurs, tabCouleurs, score);
        Arrays.sort(codeRef);

        int iFond = 0, iCurs = lgCode - 1, posCurs = 0;
        while (iFond < iCurs) {
            char[] proposition = new char[lgCode];
            for (int i = 0; i < lgCode; i++) {
                if (i == posCurs) {
                    proposition[i] = codeRef[iCurs];
                } else {
                    proposition[i] = codeRef[iFond];
                }
            }
            int[] res = score(proposition, lgCode, nbCouleurs, tabCouleurs);
            if (res[0] == 0) {
                posCurs--;
            } else if (res[0] == 1) {
                posCurs++;
    ...
    public static char[] mancheOrdi(int lgCode, int nbCouleurs, char[] tabCouleurs, int[] score) {
        int[] compteurs = initTab(nbCouleurs, 0);
        int[] proposition = new int[lgCode];
        int i = 0;
        while (i < lgCode && score[1] < lgCode) {
            int c = (int) (Math.random() * nbCouleurs);
            if (compteurs[c] < lgCode) {
                proposition[i] = c;
                compteurs[c]++;
                i++;
            }
        ...
        while (i < lgCode) {
            proposition[i] = (int) (Math.random() * nbCouleurs);
            i++;
        }
        char[] p = new char[lgCode];
        for (i = 0; i < lgCode; i++) p[i] = tabCouleurs[proposition[i]];
        return p;
    }

    public static int[] score(char[] proposition, int lgCode, int nbCouleurs, char[] tabCouleurs) {
        int bienPlace = 0;
        char[] secret = getSecret(lgCode, nbCouleurs, tabCouleurs);
        for (int i = 0; i < lgCode; i++) {
            if (proposition[i] == secret[i]) {
                bienPlace++;
                proposition[i] = secret[i] = ' ';
            }
        }
        int malPlace = 0;
        for (int i = 0; i < lgCode; i++) {
            if (estPresent(secret, proposition[i])) {
                malPlace++;
                secret[plusGrandIndice(secret, proposition[i])] = ' ';
            }
        }
        return new int[]{bienPlace, malPlace};
    }

    public static char[] getSecret(int lgCode, int nbCouleurs, char[] tabCouleurs) {
        int[] ca = codeAleat(lgCode, nbCouleurs);
        char[] secret = new char[lgCode];
        for (int i = 0; i < lgCode; i++) secret[i] = tabCouleurs[ca[i]];
        return secret;
    }
