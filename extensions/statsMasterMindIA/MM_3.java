...
public class MM_3 {
    ...
    public static void statsMasterMindIA(int lgCode, int nbCouleurs, char[] tabCouleurs, int nbEssais) {
        ...
        int sum = 0;
        int nbMaxCoups = 0;
        int nbCodes = (int) Math.pow(nbCouleurs, lgCode);
        for (int i = 0; i < nbCodes; i++) {
            int[] tabTemp = new int[lgCode];
            for (int j = 0; j < i; j++) passeCodeSuivantLexico(tabTemp, nbCouleurs);
            int nbCoups = nbCoupsStat(tabTemp, lgCode, tabCouleurs, nbCouleurs, nbEssais);
            if (nbCoups > max) {
                max = nbCoups;
                nbMaxCoups = 1;
        ...
        System.out.println("Moyenne des nombres de propositions de codes : " + (double) sum / nbCodes);
        System.out.println("═════════════════════════════════════════════════════════════════════════════════════════");
    }
